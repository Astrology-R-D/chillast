...
const LocationResolver = require('../src/core/location/LocationResolver');
...
test('resolves IANA zones and historical offsets without network access', () => {
  ...
  assert.deepEqual(summer, {
    timeZone: 'America/New_York', offsetMinutes: -240, utcOffset: 'UTC-04:00', instantUtc: '2024-07-01T16:00:00.000Z',
  });
  assert.equal(winter.offsetMinutes, -300);
  assert.equal(winter.utcOffset, 'UTC-05:00');
  assert.equal(beijing.timeZone, 'Asia/Shanghai');
  assert.equal(beijing.offsetMinutes, 480);
});
...
test('matches the existing chart conversion for ambiguous fall-back local times', () => {
  ...
  assert.equal(result.offsetMinutes, -240, 'Luxon/chart casting selects the earlier EDT occurrence');
});
