...
- ✅ **阶段 A 已落地**：`ToolProvider` + `ToolRegistry` + `AstroToolProvider` / `ContextToolProvider` / `ProfileToolProvider` / `KnowledgeToolProvider`；`AiService` 改为从 Registry 取工具，并用 LangGraph ReAct Agent 接管 chat 的工具调用循环；RAG 从 `ChainFactory` 内联工具迁移为 `KnowledgeToolProvider`。Agent 工具调用已在真实模型上联网验证通过。
- ✅ **阶段 B 已落地**：`@langchain/mcp-adapters@0.6.0`（兼容 core 0.3.x）+ `McpManager` + `McpToolProvider`，配置驱动（`ai.mcpServers`）。`McpManager` 仅连接 `enabled:true` 的服务器，`@langchain/mcp-adapters` 走 `esm-bridge` 懒加载且失败容错（导入或某服务器失败则降级为无 MCP，不影响应用）。默认 `mcpServers:{}` → MCP 休眠。`tests/McpManager.test.mjs` 5 项通过。
  - ⚠️ **依赖锁定**：MCP SDK 必须锁 `@modelcontextprotocol/sdk@1.12.3`（已加为直接依赖）。SDK 1.29 在 zod 3.25 下会因 union schema 报错；而 `zod-to-json-schema` 又要求 zod ≥3.25（`/v3` 子路径）。两者的兼容窗口即 zod 3.25.76 + SDK 1.12.3。升级 zod / SDK 前需重新验证。
  - ⚠️ 外部 MCP 服务器的**实际连接/工具调用**需配置真实服务器后联网验证。
- ✅ **阶段 C 已落地**：AI 设置新增「工具与 MCP」分区——列出各 Provider 及其工具、按来源启停（持久化到 `toolProviders`）；MCP 服务器增/删/启停 + 保存（持久化到 `mcpServers`，经 `AiService.updateMcp` 实时生效，启用外部服务器有安全确认）。IPC：`ai:tools:describe` / `ai:tools:setProviderEnabled` / `ai:mcp:list` / `ai:mcp:save`；`ai:configure` 改为合并写入以免互相覆盖。
  - ⚠️ 面板交互需在运行的应用内人工验证（本环境无法跑 Electron UI）。
- 📄 **附：Tools→MCP 迁移研究** 见 `2026-06-21-tools-to-mcp-migration-research.md`：结论为不全量迁移，实时状态工具留进程内；可选把无状态计算能力发布为独立 `astrology-mcp` 服务器供外部复用。
